 ...
 */
export function createFetchMock(mockResponses: Record<string, (url: string, options?: RequestInit) => Promise<Response>>) {
  return async (input: RequestInfo | URL, options?: RequestInit): Promise<Response> => {
    ...
    return new Response(JSON.stringify({ error: "No mock response configured for this URL" }), {
      status: 404,
 ...
 */
export function mockJsonResponse(data: any, options?: { status?: number, headers?: Record<string, string> }) {
 ...
 * mockFetch.restore();
 */
export function setupFetchMock(mockResponses: Record<string, (url: string, options?: RequestInit) => Promise<Response>>) {
