...
describe("refreshToken", () => {
  it("should refresh token successfully", async () => {
    ...
    try {
      const result = await refreshToken("client_id", "client_secret", "refresh_token");
  ...
  it("should throw HTTPException on error", async () => {
    ...
    try {
      await expect(refreshToken("client_id", "client_secret", "invalid_token")).rejects.toThrow(HTTPException);
    } finally {
      globalThis.fetch = originalFetch;
    }
  });
