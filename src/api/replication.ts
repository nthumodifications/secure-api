...
import type { EventDocType, TimetableSyncDocType, FirestoreCheckpointType } from "../utils/firestore_replication/firestore_replication_types";
...
const app = new Hono()
  .get("/events/pull",
    ...
    async (c) => {
      ...
      const documents: WithDeleted<EventDocType>[] = useDocs
  ...
  .post("/events/push",
    ...
    requireAuth(["calendar"]),
    async (c) => {
      const rows = c.req.valid("json") as RxReplicationWriteToMasterRow<EventDocType>[];
      const user = c.var.user;
      ...
      await adminFirestore.runTransaction(async (_tx) => {
        ...
        const docsInDbResult = await getContentByIds<EventDocType>(docIds, getQuery);
    ...
    .get("/timetablesync/pull",
      ...
      async (c) => {
        ...
        let newerQuery: Query;
        let sameTimeQuery: Query | undefined;
        const pullQuery = adminFirestore.collection("users").doc(user.userid).collection("timetablesync");
        ...
        const lastDoc = ensureNotFalsy(lastOfArray(useDocs));
        const documents: WithDeleted<TimetableSyncDocType>[] = useDocs
          .map(row => firestoreRowToDocData(
            'serverTimestamp',
            'semester',
            row
    ...
    .post("/timetablesync/push",
      ...
      async (c) => {
        ...
        await adminFirestore.runTransaction(async (_tx) => {
          ...
          const docsInDbResult = await getContentByIds<TimetableSyncDocType>(docIds, getQuery);
          ...
          await Promise.all(
            Object.entries(writeRowsById).map(async ([docId, writeRow]) => {
              const docInDb: TimetableSyncDocType | undefined = docsInDbById[docId];
              if (
                docInDb &&
                (
                  !writeRow.assumedMasterState ||
